...
// The files whose content feeds other stylesheets: a modern theme's palette
// (variables.css) and a legacy Less theme's variable contract. Exact basenames,
// matching core's getThemeVariablesPaths -- a package-local variables.css in an
// ordinary package is just a stylesheet, since custom properties cascade at
// runtime.
const THEME_VARIABLES_BASENAMES = new Set([
  "variables.css",
  "ui-variables.less",
...
module.exports = class PackageWatcher extends Watcher {
  ...
  loadStylesheet(pathName) {
    this.queueReload(
      this.pack.getType() === "theme" &&
        THEME_VARIABLES_BASENAMES.has(path.basename(pathName).toLowerCase()),
    );
  }
  ...
  queueReload(globalsChanged) {
    ...
    this.scheduleReload(() => {
      if (this.globalsChanged) {
        ...
        this.emitGlobalsChanged(this.pack);
      } else {
  ...
  // Whether any of the package's stylesheets compile against the theme
  // variables. A nested .less partial only matters when a stylesheet on this
  // list imports it, and CSS cannot import Less, so the attached set decides.
  consumesLess() {
    return this.pack
      .getStylesheetPaths()
      .some((stylesheetPath) => stylesheetPath.toLowerCase().endsWith(".less"));
  }
};
